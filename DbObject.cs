...
namespace Monocle
{
    ...
    public abstract class DbObject
    {
        ...
        private static void SetProperty<T>(T objInstance, IDictionary<string, int> cols, object[] objData, PropertyDescriptor prop) where T : new()
        {
            ...
            catch (Exception ex)
            {
                if (ex is FormatException || ex is InvalidCastException)
                    throw new InvalidCastException("Could not cast value in column '" + prop.Name + "' to type " + prop.PropertyType.Name + ". Value was: " + drVal + ".", ex);
                    
                throw;
            }
        }

        private static Dictionary<string, int> CacheColumnDefinitions(IDataRecord reader, string cmdText)
        {
            Dictionary<string, int> cols;

            if (!CachedColumnDefs.TryGetValue(cmdText, out cols))
            {
        ...
        internal static T FromParameters<T>(IDataReader reader, string command) where T : new()
        {
            var type = typeof(T);
            ...
            var cols = CacheColumnDefinitions(reader, command);
        ...
        internal static IEnumerable<T> ListFromParameters<T>(IDataReader reader, string command) where T : new()
        {
            while (reader.Read())
            {
                yield return FromParameters<T>(reader, command);
            }
